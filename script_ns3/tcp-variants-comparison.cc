...
{
        // 3 experimentos para: TcpTahoe, TcpNewReno, TcpWestwood com 5 repetições cada
        // taxa de erro aumentando de 2 em 2%
        // ativar o tipo trace

/*
  std::string transport_prot = "TcpWestwood"; // alterar
  double error_p = 0.0; // 
  std::string bandwidth = "2Mbps";
  std::string access_bandwidth = "10Mbps";
  std::string access_delay = "45ms";
  bool tracing = true;
  std::string tr_file_name = "teste";
  std::string cwnd_tr_file_name = "teste_";
  std::string ssthresh_tr_file_name = "teste__";
...
*/
  ...
  // TcpTahoe, TcpNewReno, TcpWestwood
  ...
  std::string access_delay = "45ms";
  bool tracing = true;
  std::string tr_file_name = "teste.tr";
  std::string cwnd_tr_file_name = "cwnd_tr_name.tr";
  std::string ssthresh_tr_file_name = "teste_sss_";
  double data_mbytes = 1000;
  uint32_t mtu_bytes = 1500;
  uint16_t num_flows = 1;
  float duration = 30;
  uint32_t run = 0;
  bool flow_monitor = true;
  std::string flow_monitor_file = "flowMonitor";
  ...
  cmd.AddValue("flow_monitor_file", "Name of output flowmonitor", flow_monitor_file);
  cmd.Parse (argc, argv);
  ...
  for (int i=0; i<num_flows; i++)
    {
      ...
      devices = LocalLink.Install(sources.Get(i), gateways.Get(i));
      address.NewNetwork();

      Ipv4InterfaceContainer interfaces = address.Assign (devices);
      devices = UnReLink.Install(gateways.Get(i), sinks.Get(i));
      address.NewNetwork();
  ...
  if (tracing)
    {
      ...
      //stack.EnableAsciiAll (ascii_wrap);

      //UnReLink.EnableAsciiAll (ascii_wrap);      
      LocalLink.EnableAsciiAll (ascii_wrap);
  ...
  */
  ...
  flowMonitor->SerializeToXmlFile(flow_monitor_file, true, true);

  /*
  if (flow_monitor)
    {
      flowHelper.SerializeToXmlFile("TcpVariantsComparison.flowmonitor" + run, true, true);
    }
  */
  Simulator::Destroy ();
  return 0;
